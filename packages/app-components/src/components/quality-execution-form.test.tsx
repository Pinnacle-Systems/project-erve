...
describe('QualityExecutionForm shared web/mobile renderer', () => {
  ...
  it('offers Start reinspection immediately on a failed batch, with no Factory rework state involved', async () => {
    const item = finalExecution('FINALIZED');
    item.finalBatch = {
      id: 'batch-2',
      batchNumber: 2,
    ...
    expect(container.textContent).not.toContain('rework');
    const startButton = button('Start reinspection');
