...
async function fixture(): Promise<Fixture> {
  ...
  async function job(): Promise<{ jobId: string; allocationId: string }> {
    ...
    await prisma.jobOrder.create({ data: { id: jobId, jobOrderNumber: `JO-${createId()}`, factoryId: factory.id, processFlowVersionId: flow.versions[0]!.id, unitPrice: '1', status: 'READY_FOR_QA', preparedQuantityTotal: 10, createdBy: user.userId, financialYearId: financialYear.id, jobOrderSerial, lines: { create: { id: jobLineId, styleId: style.id, orderedQuantityTotal: 10, preparedQuantityTotal: 10, status: 'READY_FOR_QA', sizes: { create: { id: allocationId, sizeId: size.id, orderedQuantity: 10, preparedQuantity: 10 } } } } } });
