...
async function createSeedGraph() {
  const admin = await createTestUserAndToken({ email: 'admin-job@test.local', password: 'pass', roles: ['ADMIN'] });
  ...
  const sizeA = await prisma.size.create({ data: { id: createId(), code: 'AGE_3', label: '3', sizeType: 'AGE', sortOrder: 3 } });
  const sizeB = await prisma.size.create({ data: { id: createId(), code: 'AGE_4', label: '4', sizeType: 'AGE', sortOrder: 4 } });
  ...
  const poRes = await request(app)
    ...
    .send({
      ...
      lines: [{ styleId: style.id, sizes: [{ sizeId: sizeA.id, orderedQuantity: 10 }, { sizeId: sizeB.id, orderedQuantity: 5 }] }],
    });
  const po = poRes.body.data;
  await request(app).post(`/purchase-orders/${po.id}/actions/submit`).set('Authorization', `Bearer ${admin.token}`);
...
async function createJobOrder(token: string, graph: Awaited<ReturnType<typeof createSeedGraph>>, quantity = 4) {
  return request(app)
    .post('/job-orders')
    ...
    .send({
      ...
      lines: [{ purchaseOrderLineId: graph.poLineId, sizes: [{ purchaseOrderLineSizeId: graph.poSizeAId, quantity }] }],
    });
}
...
describe('job orders API', () => {
  it('creates a draft job order, increments PO balance, rolls PO status, and writes audit', async () => {
    ...
    const poSize = await prisma.distributorPurchaseOrderLineSize.findUniqueOrThrow({ where: { id: graph.poSizeAId } });
    ...
    await expect(prisma.auditLog.count({ where: { action: 'JOB_ORDER_CREATED' } })).resolves.toBe(1);
  });

  it('rejects DRAFT PO, inactive factory, inactive flow, excess quantity, wrong line, and wrong size', async () => {
    const graph = await createSeedGraph();
    const draftPo = await prisma.distributorPurchaseOrder.update({ where: { id: graph.poId }, data: { status: 'DRAFT' } });
    await expect(createJobOrder(graph.admin.token, graph)).resolves.toMatchObject({ status: 400 });
    await prisma.distributorPurchaseOrder.update({ where: { id: draftPo.id }, data: { status: 'SUBMITTED' } });
    ...
    await expect(
      request(app).post('/job-orders').set('Authorization', `Bearer ${graph.admin.token}`).send({
        purchaseOrderId: graph.poId,
        factoryId: graph.factory.id,
        processFlowVersionId: graph.draftProcessFlowVersionId,
        lines: [{ purchaseOrderLineId: graph.poLineId, sizes: [{ purchaseOrderLineSizeId: graph.poSizeAId, quantity: 1 }] }],
      }),
    ).resolves.toMatchObject({ status: 400 });

    await expect(createJobOrder(graph.admin.token, graph, 99)).resolves.toMatchObject({ status: 400 });
    await expect(
      request(app).post('/job-orders').set('Authorization', `Bearer ${graph.admin.token}`).send({
        purchaseOrderId: graph.poId,
        factoryId: graph.factory.id,
        processFlowVersionId: graph.processFlowVersionId,
        lines: [{ purchaseOrderLineId: createId(), sizes: [{ purchaseOrderLineSizeId: graph.poSizeAId, quantity: 1 }] }],
      }),
    ).resolves.toMatchObject({ status: 400 });
    await expect(
      request(app).post('/job-orders').set('Authorization', `Bearer ${graph.admin.token}`).send({
        ...
        factoryId: graph.factory.id,
        processFlowVersionId: graph.processFlowVersionId,
        lines: [{ purchaseOrderLineId: graph.poLineId, sizes: [{ purchaseOrderLineSizeId: createId(), quantity: 1 }] }],
      }),
    ).resolves.toMatchObject({ status: 400 });
  });
  ...
  it('moves PO to fully job ordered when all quantities are consumed', async () => {
    const graph = await createSeedGraph();
    const res = await request(app).post('/job-orders').set('Authorization', `Bearer ${graph.admin.token}`).send({
      purchaseOrderId: graph.poId,
      factoryId: graph.factory.id,
      processFlowVersionId: graph.processFlowVersionId,
      lines: [
  ...
  it('runs send, confirm, stage completion, prepared quantity, and variance workflow', async () => {
    ...
    const sendRes = await request(app).post(`/job-orders/${jobOrderId}/actions/send-to-factory`).set('Authorization', `Bearer ${graph.admin.token}`);
    expect(sendRes.status).toBe(200);
    expect(sendRes.body.data.status).toBe('SENT_TO_FACTORY');
    await expect(request(app).post(`/job-orders/${jobOrderId}/actions/send-to-factory`).set('Authorization', `Bearer ${graph.admin.token}`)).resolves.toMatchObject({ status: 400 });

    const factoryUser = await createTestUserAndToken({ email: 'factory-job@test.local', password: 'pass', roles: ['FACTORY_USER'] });
    await prisma.userFactory.create({ data: { id: createId(), userId: factoryUser.userId, factoryId: graph.factory.id } });
    const otherFactoryUser = await createTestUserAndToken({ email: 'other-factory-job@test.local', password: 'pass', roles: ['FACTORY_USER'] });
    await prisma.userFactory.create({ data: { id: createId(), userId: otherFactoryUser.userId, factoryId: graph.otherFactory.id } });

    await expect(request(app).post(`/job-orders/${jobOrderId}/actions/confirm`).set('Authorization', `Bearer ${otherFactoryUser.token}`)).resolves.toMatchObject({ status: 403 });
    const confirmRes = await request(app).post(`/job-orders/${jobOrderId}/actions/confirm`).set('Authorization', `Bearer ${factoryUser.token}`);
    ...
    await expect(
      request(app).post(`/job-orders/${jobOrderId}/actions/complete-stage`).set('Authorization', `Bearer ${factoryUser.token}`).send({ stageStatusId: stages[1].id }),
    ).resolves.toMatchObject({ status: 400 });
    ...
    await expect(
      request(app)
        ...
        .set('Authorization', `Bearer ${factoryUser.token}`)
        .send({ sizes: [{ jobOrderLineSizeId: createRes.body.data.lines[0].sizes[0].id, preparedQuantity: 3 }] }),
    ).resolves.toMatchObject({ status: 400 });
    ...
    const varianceRes = await request(app).get(`/job-orders/${jobOrderId}/variance`).set('Authorization', `Bearer ${graph.admin.token}`);
  ...
  it('blocks distributor access to job orders', async () => {
    ...
    const distributorUser = await createTestUserAndToken({ email: 'dist-job@test.local', password: 'pass', roles: ['DISTRIBUTOR'] });
    await prisma.userDistributor.create({ data: { id: createId(), userId: distributorUser.userId, distributorId: graph.distributor.id } });

    const listRes = await request(app).get('/job-orders').set('Authorization', `Bearer ${distributorUser.token}`);
    const detailRes = await request(app).get(`/job-orders/${createRes.body.data.id}`).set('Authorization', `Bearer ${distributorUser.token}`);
