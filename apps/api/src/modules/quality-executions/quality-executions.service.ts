  ...
  const saved = await prisma.$transaction(async (tx) => {
    ...
    if (finalize && execution.processFlowActivity.executionMultiplicity === 'BATCHED') {
      ...
      if (input.outcome?.value === 'PASS') {
        ...
        // Physical correction of a FAILed batch happens with the Factory,
        // outside ERVE. The batch itself (disposition + retained attempt
        // history) carries all the state a reinspection needs; ERVE does
        // not track or gate on any internal corrective-action workflow.
        await tx.finalQualityBatch.update({
          where: { id: batch.id },
          data: {
