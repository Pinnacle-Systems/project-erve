    ...
    data: {
      ...
      lines: {
        create: {
          id: createId(),
          styleId: style.id,
          sizes: { create: { id: createId(), sizeId: size.id, orderedQuantity: 840 } },
        },
      },
    },
...
const start = (f: Awaited<ReturnType<typeof fixture>>, quantity: number) =>
  request(app)
...
describe('Final Inspection batching and prepared coverage', () => {
  ...
  it('does not count drafts, rejects over-inspection, and surfaces later prepared-quantity changes', async () => {
    ...
    const increased = await request(app)
      .get(`/quality-executions/${first.id}`)
      .set('Authorization', `Bearer ${f.qa.token}`)
      .expect(200);
    expect(increased.body.data.coverage).toMatchObject({
    ...
    const lowered = await request(app)
      ...
      .set('Authorization', `Bearer ${f.qa.token}`)
      .expect(200);
    expect(lowered.body.data.coverage).toMatchObject({
      inspectedQuantity: 800,
      remainingQuantity: 0,
  ...
  it('serializes concurrent finalizations so cumulative coverage cannot overrun prepared quantity', async () => {
    ...
    const two = await prisma.qualityActivityExecution.create({
      data: {
        id: createId(),
        jobOrderId: f.job.id,
        processFlowActivityId: f.final.id,
        qualityFormVersionId: f.form.versions[0]!.id,
        attemptNumber: 1,
        batchNumber: 2,
        inspectedQuantity: 50,
        startedById: f.qa.userId,
      },
    });
    const [a, b] = await Promise.all([finalize(f, one), finalize(f, two)]);
    expect([a.status, b.status].sort()).toEqual([200, 400]);
    const aggregate = await prisma.qualityActivityExecution.aggregate({
      where: { jobOrderId: f.job.id, processFlowActivityId: f.final.id, status: 'FINALIZED' },
