...
describe('PoweredByPinnacleBranding (web)', () => {
  ...
  it('compact variant selects the theme-appropriate mark asset, not the full lockup', () => {
    ...
    const img = container.querySelector('img') as HTMLImageElement;
    expect(img.getAttribute('src')).toBe('/pinnacle-mark-on-light.png');
  });
  ...
  it('compact variant follows dark theme too', () => {
    ...
    renderBranding('compact');
    const img = container.querySelector('img') as HTMLImageElement;
    expect(img.getAttribute('src')).toBe('/pinnacle-mark-on-dark.png');
  });
});
