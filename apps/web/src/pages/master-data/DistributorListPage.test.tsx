...
describe('DistributorListPage PDF actions', () => {
  it('shows Download PDF and Print actions near the filter bar', async () => {
    await renderPageWithDistributors([]);
  ...
  it('shows an inline error and re-enables the actions if generation fails', async () => {
    await renderPageWithDistributors([makeDistributor()]);
  ...
  it('downloads the distributor list PDF under the expected filename', async () => {
    await renderPageWithDistributors([makeDistributor()]);
    ...
    expect(downloadSpy.mock.calls[0]![1]).toMatch(/^ERVE-Distributors-\d{4}-\d{2}-\d{2}\.pdf$/);
  });
