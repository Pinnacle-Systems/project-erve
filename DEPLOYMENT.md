...
| `/api/ready` returns 503 | Real DB connectivity issue — check `psql "$DATABASE_URL" -tAc "SELECT 1;"` on the VPS directly (same command `deploy-release.sh` and `backup-database.sh` use) |
