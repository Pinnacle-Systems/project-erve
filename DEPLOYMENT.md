...
| `package-production.sh` fails during `pnpm deploy` for the API artifact           | Production dependencies are installed via `pnpm --filter @erve/api deploy <dir> --prod --legacy`, resolved from the repository's own frozen `pnpm-lock.yaml` (never re-resolved against the live registry) — check `apps/api/package.json`'s `dependencies` and re-run `pnpm install` at the repo root to refresh the lockfile if you've added/changed a dependency; `prisma` must stay a real `dependency` (not `devDependency`), since `--prod` excludes devDependencies and the VPS needs `node_modules/.bin/prisma` to run migrations |
| `package-production.sh` fails a "Checkpoint: verifying runtime dependencies" step | `verify-artifact-deps.sh` found a package (most commonly a deep transitive dependency like `iconv-lite`) that resolved but is missing files underneath, or doesn't function correctly — this is the exact class of bug that once let an incomplete `iconv-lite` (present `lib/`, missing `encodings/`) reach production; investigate the specific package named in the failure rather than working around the check                                                                                                                       |
...
| Deployment/rollback fails with "PM2 target assertion FAILED"                      | `erve_assert_pm2_release` (`scripts/deployment/lib/common.sh`) found that `pm2 jlist`'s `pm_exec_path`/`pm_cwd` for `erve-api` don't match the release being activated — either PM2 is absent, ambiguous (more than one `erve-api` process), or still bound to a different release. This is the exact class of bug that used to leave PM2 silently serving a stale/deleted release while `current` and `pm2 status` both looked fine; `erve_activate_pm2_release` deletes and freshly starts `erve-api` directly against the target release before every health check specifically so this can't happen. Do not "fix" this by running `pm2 restart`/`pm2 reload` by hand — they reuse PM2's already-stored script path/cwd rather than re-resolving it; investigate why an out-of-band `pm2 start`/`pm2 restart` happened instead                                                                                                                                                                                                                                                                                                                                                                                                        |
